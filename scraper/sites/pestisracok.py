"""
Custom scraper for PestiSrácok
"""
import requests
...
class PestiSracokScraper:
    def __init__(self, max_age_hours=24):
        self.base_url = "https://pestisracok.hu"
    ...
    def fetch_articles(self, max_articles=10):
        """Fetch articles from PestiSrácok homepage"""
        articles = []
        
        try:
            logger.info(f"Scraping PestiSrácok")
            response = requests.get(self.base_url, headers=self.headers, timeout=30)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Try finding articles by common patterns
            ...
            if not article_elements:
                # Try finding links to articles
                article_elements = soup.find_all('a', href=lambda x: x and ('/hir/' in x or '/cikk/' in x or '/cimke/' not in x), limit=max_articles * 2)
            ...
            for element in article_elements:
                try:
                    # Find title
                    title_elem = element.find(['h1', 'h2', 'h3', 'h4', 'span', 'a'])
                    if not title_elem:
                        continue
            ...
            logger.info(f"Scraped {len(articles)} articles from PestiSrácok")
            
        except Exception as e:
            logger.error(f"Error scraping PestiSrácok: {e}")
