...
class PartizanScraper:
    def __init__(self, max_age_hours=24):
        self.base_url = "https://partizan.hu"
    ...
    def fetch_articles(self, max_articles=10):
        """Fetch articles from Partizán homepage"""
        articles = []
        
        try:
            logger.info(f"Scraping Partizán")
            ...
            if not article_elements:
                # Try finding links
                article_elements = soup.find_all('a', href=lambda x: x and ('/hir/' in x or '/cikk/' in x or '/videok/' not in x), limit=max_articles * 2)
            ...
            logger.info(f"Scraped {len(articles)} articles from Partizán")
            
        except Exception as e:
            logger.error(f"Error scraping Partizán: {e}")
